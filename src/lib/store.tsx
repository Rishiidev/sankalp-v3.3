...
export interface UserData {
  ...
  youtubeTracks?: { id: string; name: string; url: string }[];
}
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (userData) {
        ...
        const updatedUser: UserData = { ...userData, streak: newStreak, lastActiveDate: new Date().toISOString() };
        if (diff > 0) {
          await saveUser(updatedUser);
        }
        setUser(updatedUser);
        const themeClass = updatedUser.theme && updatedUser.theme !== 'midnight' ? `theme-${updatedUser.theme}` : '';
        const deityClass = updatedUser.deity ? `deity-${updatedUser.deity}` : 'deity-hanuman';
      ...
      let presetsData = await getMantraPresets();
      const defaults = [
      ...
      for (const def of defaults) {
        if (!presetsData.some(p => p.id === def.id)) {
          presetsData.push(def);
      ...
      if (challengesData.length === 0) {
        ...
        setChantChallenges([newChallenge]);
      } else {
  ...
  const updateUser = async (data: Partial<UserData>) => {
    ...
    if (data.theme !== undefined || data.deity !== undefined) {
      const themeClass = updated.theme && updated.theme !== 'midnight' ? `theme-${updated.theme}` : '';
      const deityClass = updated.deity ? `deity-${updated.deity}` : 'deity-hanuman';
      document.documentElement.className = [themeClass, deityClass].filter(Boolean).join(' ');
    }
  };

  const updateMalaCount = async (count: number) => {
    if (!user) return;
    await updateUser({ currentMalaCount: count });
  };

  const toggleHaptics = async () => {
    if (!user) return;
    await updateUser({ hapticsEnabled: !user.hapticsEnabled });
  };
  ...
  const addYoutubeTrack = async (name: string, url: string) => {
    if (!user) return null;
    const id = `yt_${Date.now()}`;
    const newTrack = { id, name, url };
    const newYoutubeTracks = [...(user.youtubeTracks || []), newTrack];
    await updateUser({ youtubeTracks: newYoutubeTracks });
    return newTrack;
  };

  const removeYoutubeTrack = async (id: string) => {
    if (!user) return;
  ...
  const editChantChallengeDetails = async (id: string, name: string, target: number, deadline: string | undefined) => {
    const challenge = chantChallenges.find(c => c.id === id);
    if (!challenge) return;
